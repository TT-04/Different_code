...
code_A equ 32					; 48 это код нуля в ASCII
...
_start:							; метка - точка входа, аналог int main() в Си
	call asm_custom_read				; вызов функции без аргументов, чтобы считать введенное посимвольно число в регистр rax
							; можно работать со считанным числом
					; вывести посимвольно число, хранящееся в rax
	call asm_exit					; wrapper - функция, в которую "завернут" стандартный набор инструкций для выхода из системы

asm_custom_read:					; объявление новой функции, похоже на atoi() в Си
	...
	l_rd:						; метка - начало цикла
							; эти инструкции, чтобы умножить rax на 10, не учтено, что может произойти переполнение,
		...
		xor rbx, rbx				; rbx=0
		mov ebx, [var_read + rdx]		; положили следующий по ходу символ в bl, 
							; [var_read + rdx] - обращение к значению, лежащему по адресу var_read + смещение из rdx
		
		mov esi, 97
		cmp ebx, esi 
		jae it_s_small_symbol
		jmp it_s_other_symbol
it_s_small_symbol: 
		mov esi, 122 
		cmp ebx, esi 
		jle print_small_symbol
		jmp it_s_number
print_small_symbol:
		sub ebx, code_A
		mov [var_write + rdx], ebx 
		loop l_rd
